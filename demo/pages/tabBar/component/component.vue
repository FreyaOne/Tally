		...
		<view class="content">
			<view style="width: 92%;">
				<view>
					...
					<view class="expenditure" v-for="(row,index) in expenditure" @tap="select(row)" style="margin-top: 20upx;" v-show="row.money >=0  && current=== 0">
						<view class="uni-padding-wrap uni-common-mt">
					...
					<view class="expenditure" v-for="(row,index) in income" @tap="select(row)" style="margin-top: 20upx;" v-show="row.money < 0 && current === 1">
						<view class="uni-padding-wrap uni-common-mt">
							<view class="uni-flex uni-row">
								<!-- 记录左部分（内容 时间）占位70%，右侧（价格类别）为30% -->
								<view class="detail-left" >
